...
func (user *User) Get() *errors.RestErr {
	if users_db.Client == nil {
		return errors.NewInternalServerError("Database connection is not initialized")
	}

	stmt, err := users_db.Client.Prepare(queryGetUser)
	if err != nil {
		// log.Printf("Error preparing query: %s", err.Error()) // Debugging log
...
func (user *User) Save() *errors.RestErr {
	...
	if err != nil {
		logger.Error("error when trying to get last inser id after creating a new user", err)
		return errors.NewInternalServerError("database error")
		// return mysql_utils.ParseError(err)
	}
...
func (user *User) FindByStatus(status string) ([]User, *errors.RestErr) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&user.Id, &user.FirstName, &user.LastName, &user.Email, &user.DateCreated, &user.Status); err != nil {
			...
			logger.Error("error when trying to scan user row into user struct in finding user by status", err)
			return nil, errors.NewInternalServerError("database error")
		}
